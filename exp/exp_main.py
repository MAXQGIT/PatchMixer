...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, PatchMixer, SegRNN
...
def predict_true_plot(pre,true,pred_len,f_dim):
    true = true.detach().cpu().numpy()
    true = true[:, -pred_len:, f_dim:]
    plt.figure()
    plt.plot(pre.reshape(96).tolist(),label ='预测值')
    plt.plot(true.reshape(96).tolist(),label ='真实值')
    plt.title('预测值和真实值对比')
    plt.legend()
    plt.savefig('./results/预测值和真实值对比.png')
...
class Exp_Main(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if any(substr in self.args.model for substr in ['Linear', 'TST', 'SegRNN', 'PatchMixer']):
    ...
    def predict(self, setting, load=False):
        ...
        pre_data = pd.DataFrame(preds)
        pre_data.to_csv(folder_path + 'result.csv', encoding='gbk')
        return
